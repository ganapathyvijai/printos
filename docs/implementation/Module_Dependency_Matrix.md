...
- **Architecture Review Dependency** — A module is blocked or delayed by an open Architecture Review Register item. Example: AR-005 Quotation, AR-006 Material/Product Template (AR-004 Machine was an example of this category until its Resolution, 2026-09-20).
...
- Architecture Review dependencies: AR-005 (Quotation, indirect via Sales Order standard path); AR-006 reaches Production Planning indirectly, through the Job Cards and Machine Scheduling inputs it consumes (AR-004 exposure removed — Resolved, Option C, 2026-09-20)
- Directly blocked: No — Production Planning as a capability has no Architecture Review item of its own.
- Transitively delayed: Yes — via Sales/Estimation's AR-005 exposure on the standard path, and via its consumption of Job Cards and Machine Scheduling (which carry AR-006 exposure).
...
- Configuration dependencies: Workflow Designer (optional)
- Plugin dependencies: None required; MachineIQ is additive only
- Architecture Review exposure: AR-006 (Material — convergent input, via allocation to tier B), AR-005 (Quotation — transitive, via Sales Order standard path), AR-009 (Quality Check Record naming/scope — see Notes). AR-004 (Machine) exposure is Resolved (Option C, 2026-09-20) and no longer a convergent input. These are convergent inputs and transitive exposure to end-to-end operation, not direct blockers of the Job Card entity itself.
...
- Directly blocked: No
- Transitively delayed: Yes — via Job Cards' convergent blocking (AR-005/AR-006 chain; AR-004 exposure Resolved).
...
- **Job Cards convergence risk (updated).** AR-004 is now Resolved (Option C, 2026-09-20); Job Cards' convergence point now depends on AR-005 and AR-006 alone. Any one of these resolving late continues to have an outsized effect on the entire Production capability (Production Planning, Job Cards, and Machine Scheduling together), disproportionate to what a simple count of "one AR item per entity" would suggest.
