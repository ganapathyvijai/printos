...
Lifecycle position of Version 1.9:
**Draft, Version 1.9 (2026-09-12).** Version 1.9 records the Project Owner's adopted detailed policy for the non-Artwork Job Card release authority (Section 10.4 Rule 11 of the seventeen-rule Model 4 package) — **new normative design content that has received no Architecture Review and no Business Review**. Under `../Documentation_Workflow.md` Section 5, which defines **Approval** as "All required reviews passed; awaiting final Owner sign-off," and Section 7, which prohibits reaching Approval "until every applicable review lens has been explicitly passed," this document **remains at Draft, Version 1.9**; Sections 6 and 11 keep Project Owner approval distinct from those lenses, so Project Owner authorization alone does not satisfy them. Workflow Section 8 confirms **MINOR** as the correct increment for an additive, non-contradictory expansion. This is the same treatment the Project Owner expressly ruled for this document at Version 1.5 (2026-08-13) and applied again at Version 1.8 (2026-09-12). **The Approval, Version 1.7 baseline (2026-08-22) remains the last fully reviewed and Owner-approved version**, and **Targeted Architecture Review and Business Review of Section 10.4 remain Pending.** No normative Job Card rule is changed, reversed, weakened, or made ambiguous by Version 1.8 or Version 1.9. This detailed-rule adoption is distinct from, and later than, the high-level Model 4 direction recorded at Version 1.8 — adopting the direction did not itself adopt this rule's detail.
...
1. The Tenant's Artwork Authority capability is recorded as **enabled**.
2. A **locked, non-superseded, positively governed "not applicable" determination** exists on the Job Card's Sales Order — carrying its deciding actor, timestamp, and mandatory reason (Section 2.2 Rules 5, 6, 8, 9, 13 of the Artwork System Design).
...
**Explicit distinctions.** A **positive governed "not applicable" decision** (element 2) is an affirmative, audited, locked record — not an absence of anything. This is **distinct from** a **null or missing `production_artwork_set`**: this rule does **not** authorize a null-link release; a missing link is never, by itself, release evidence. This is **distinct from an informal waiver**: "not applicable" cannot be selected by the releaser (decider ≠ releaser, per Rule 5), cannot be set without a mandatory reason (Rule 6), and cannot be changed after lock without the governed correction procedure (Rule 9).

**Separation of duties preserved.** The applicability decider and the Job Card releaser are never the same authority. Holding release authority does not grant applicability-decision authority, and holding applicability-decision authority does not grant release authority — extending the existing Section 11 (Artwork System Design) separation to this path.
...
**Historical preservation, correction, and withdrawal.** A Job Card released under this authority follows the same historical-preservation rules as an Artwork-controlled Job Card: the composite evidence relied upon at release is **never modified after Release**; there is **no silent rebinding**. If the underlying applicability determination is later corrected, that correction **supersedes** the prior determination (Rule 9) but does **not** retroactively alter a Job Card already Released under the prior determination — mirroring Section 16.1/16.2's treatment of Superseded and Withdrawn Sets.
...
**Not safe for coding.** The exact non-Artwork release prerequisites at execution level, the technical mechanism for locking and revalidating the composite evidence at release time, and the global transaction lock order (Rule 17; tied to `RO-13`) all remain unresolved. Targeted Architecture Review and Business Review of this section are **Pending**. No gate is closed by this section.
