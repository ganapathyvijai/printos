...
This Registry is grounded in the domain vocabulary already established in `docs/blueprint/05_Domain_Model.md`, the bounded contexts in `docs/blueprint/06_Bounded_Contexts.md`, and the master data entities in `docs/blueprint/08_Master_Data_Model.md`. `docs/business/01_Business_Glossary.md` exists at Draft, Version 1.0 and defines business meaning, while this Registry governs which names are approved. Until the two Draft documents are formally reconciled, this Registry continues to derive its approved vocabulary from the Blueprint sources listed above.
...
Approved business terms are sourced from `docs/blueprint/05_Domain_Model.md` and `docs/blueprint/08_Master_Data_Model.md`. The existing `docs/business/01_Business_Glossary.md` (Draft, Version 1.0) has not yet been formally reconciled with this table. This Registry remains the authority for which names are approved; the Glossary defines business meaning.
...
| 10 | ERPNext "Item" vs PrintOS "Material"/"Product Template" | Clarify mapping between ERPNext core concept and PrintOS domain terms | Level 2 | **Resolved — [ADR-016](../decisions/ADR-016-Item-Material-Product-Template-Mapping.md).** Material is a Custom PrintOS master linked to native Item; Substrate is a specialization of Material; Product Template is its own Custom PrintOS artifact, independent of Material and Item. |
...
| Business Glossary | Business meaning of terms (exists at Draft, Version 1.0; formal reconciliation with this Registry remains pending) | `docs/business/01_Business_Glossary.md` |
...
## 40. AR-003 Naming Alignment Disposition

Per Project Owner decision dated 2026-09-19 (Option A), the following disposition resolves Architecture Review Register item AR-003 for all eight originally flagged terms. This section distinguishes an Approved canonical term, a permitted ordinary business/workflow label, a rejected/not-adopted module or capability name, a mapped alias, and a Proposed future capability — no requested phrase is marked Approved merely by appearing here.

| Term | Disposition | Type | Canonical / Mapped To | Notes |
|---|---|---|---|---|
| Print Specification | Not adopted | Rejected module/object name | Product Template, Job Types, Finishing Types, Paper Sizes (Approved master data, combined) | No new object created; combination already covers business need |
| Approval Management | Not adopted | Rejected module name | Approval Designer (Configuration Studio) | Category mismatch; configuration surface, not a business module |
| Production Workflow | Permitted | Ordinary business-workflow label | Production Planning + Job Cards + Machine Scheduling (orchestrated) | Not a module, DocType, service, or capability; matches existing `10_Business_Workflows.md` usage; permitted as a label only, not Approved as a module |
| Machine Management | Not adopted | Mapped alias | Machine Scheduling (Approved module) | AR-004 (Machine object base) remains separately open and unchanged |
| Finishing | Already Approved | Approved business term | "Finishing Types" master data (Sections 22/24) | Not an approved standalone module; existing Approved status unchanged by this section |
| Quality Control | Permitted | Ordinary workflow/process label + embedded Job Card capability | Quality Check Record (within Job Cards) | Permitted as a label and embedded capability only, not Approved as a standalone module; the standalone "Quality" module question remains AR-009 / §27 item 9, unresolved and unchanged |
| AI Assistant | **Proposed only** | Proposed future capability name | None — no existing analog | **Registered, not Approved.** No Approved Bounded Context. No architecture, provider, or model decision. Remains excluded from implementation-ready scope. No implementation authorization. |
| Production Orchestration | Not adopted | Rejected/duplicate name | Production Planning + Job Cards | No current live usage found anywhere in governed documents |

A "Permitted" or "Mapped alias" disposition above does not confer Approved status on the requested phrase itself; only the canonical term it maps to remains Approved. This section does not change the status of any other registered term, including the six Artwork/Job Card Section 13a technical names (all remain Under Review, unaffected). It does not resolve AR-004 or AR-009. It grants no implementation authorization.
...
- Now that `docs/business/01_Business_Glossary.md` exists at Draft, Version 1.0, this Registry's Business Vocabulary Registry (Section 6) should be formally reconciled with it, with the Glossary defining business meaning and this Registry continuing to govern which name is approved.
...
- `docs/blueprint/22_Integration_Architecture.md`, `23_Security_Architecture.md`, and `24_Deployment_Architecture.md` remain reserved and not yet created per [ADR-010-Blueprint-Numbering-Strategy.md](../decisions/ADR-010-Blueprint-Numbering-Strategy.md). `docs/blueprint/25_MultiTenant_Architecture.md` now exists at Approval, Version 1.0, but is not Published. Vocabulary from these documents should be folded into Sections 8–9 as their governed content becomes applicable.
...
- How should the existing `docs/business/01_Business_Glossary.md` (Draft, Version 1.0) be formally reconciled with and cited by this Registry as the business-meaning source?
- Which remaining Pending ADR items in the Naming Decision Matrix (Section 27) should be prioritized first, given that several affect module and entity naming?
...
|1.20|2026-09-26|AR-006 Naming Alignment Disposition|Corrected Naming Decision Matrix (Section 27) item #10 from "Pending ADR" to Resolved, citing [ADR-016-Item-Material-Product-Template-Mapping](../decisions/ADR-016-Item-Material-Product-Template-Mapping.md): Material is a Custom PrintOS master linked to native Item; Substrate is a specialization of Material; Product Template is its own Custom PrintOS artifact, independent of Material and Item. This resolves Architecture Review Register AR-006. No other Naming Decision Matrix item, Synonym Registry entry, or Term status changed by this correction. Substrate's own ERPNext DocType structure (child table vs. standalone) remains deferred to downstream design and is not specified by this entry. No implementation authorized.|
|1.19|2026-09-19|AR-003 Naming Alignment Disposition|Added Section 40, recording the Project Owner's Option A disposition (2026-09-19) for all eight AR-003 terms: seven mapped, confirmed, or rejected against existing Approved concepts; AI Assistant registered as Proposed only, with no Bounded Context, architecture, provider, model, or implementation authorization. No existing entry's status changed — the six §13a Artwork/Job Card names remain Under Review, unaffected; Finishing's existing Approved status (Sections 22/24) is unchanged, only cross-referenced. AR-004 and AR-009 are not resolved by this entry.|
|1.18|2026-09-19|Business Glossary and Blueprint 25 Reference Synchronization|Corrected active references to record that `docs/business/01_Business_Glossary.md` exists at Draft, Version 1.0 and that `docs/blueprint/25_MultiTenant_Architecture.md` exists at Approval, Version 1.0 but is not Published. Updated the stale open question claiming all original Naming Decision Matrix items remained unresolved. Reference-only correction; no Approved, Under Review, Proposed, Deprecated, or Archived term changed status; no naming or architecture decision was made; no implementation authorization was granted.|
