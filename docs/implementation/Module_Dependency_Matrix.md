...
- **Architecture Review Dependency** — A module is blocked or delayed by an open Architecture Review Register item. Example: AR-010 BOM necessity for Cost Estimate's detailed cost-breakdown design (AR-004 Machine, AR-005 Quotation Strategy, and AR-006 Material/Product Template were examples of this category until their Resolution, 2026-09-20, 2026-09-26, and 2026-09-26 respectively).
...
| MachineIQ | Machine (Custom, AR-004 Resolved), Production data — MachineIQ depends on Production, not the reverse |
| Marketplace | None technically; routes into CRM/Sales once active |
| AI Assistant | Not assessable — Proposed name only, no Bounded Context, unscoped |
...
- ERPNext dependencies: Product Template is Resolved as its own Custom PrintOS artifact, independent of Item (AR-006, Option C, 2026-09-26; ADR-016)
- Configuration dependencies: None required to bootstrap
- Plugin dependencies: None
- Architecture Review dependencies: AR-005 (Quotation strategy) is Resolved — Option B, 2026-09-26: Custom Cost Estimate and pricing logic feed ERPNext's native Quotation through a governed handoff; AR-006 (Product Template) is Resolved — Option C, 2026-09-26: Product Template is its own Custom PrintOS artifact, independent of Material and Item; AR-010 (BOM necessity — affects the Estimation Engine's internals, not entity existence) remains Open and should be resolved before detailed Cost Estimate cost-breakdown design, to avoid later rework
- Directly blocked: No — Product Template is Resolved (AR-006, Option C). Quotation and Quotation Line remain Pending Architecture Review, not because AR-005 is unresolved (it is Resolved), but because AR-010 remains the open dependency for Cost Estimate's detailed cost-breakdown design and Quotation Line's exact target is deferred to downstream DocType design. Job Types, Finishing Types, and Paper Sizes reference data are not blocked.
- Transitively delayed: Not applicable for Product Template (Resolved); Quotation/Quotation Line remain gated by AR-010 alone.
...
- Plugin dependencies: None
- Architecture Review dependencies: AR-005 is Resolved (Option B, 2026-09-26) and AR-006 is Resolved (Option C, 2026-09-26; ADR-016); Estimation's remaining AR-010 exposure reaches Sales only indirectly, for the standard Quotation-based path
- Directly blocked: No — Sales Order itself is Extended ERPNext, not Pending.
- Transitively delayed: Yes, for the standard path, via Estimation's remaining AR-010 exposure.
...
- Architecture Review dependencies: none — Artwork is not governed by AR-003; Approval Record's own naming remains a separate, currently unassigned governance question, distinct from the "Approval Management" module-name mapping AR-003 resolved
- Directly blocked: No
...
- Architecture Review dependencies: AR-005 (Quotation) is Resolved (Option B, 2026-09-26) and no longer an exposure via the Sales Order standard path; AR-006 (Material) is Resolved (Option C, 2026-09-26; ADR-016) and no longer an indirect exposure through the Job Cards and Machine Scheduling inputs it consumes (AR-004 exposure removed — Resolved, Option C, 2026-09-20)
- Directly blocked: No — Production Planning as a capability has no Architecture Review item of its own.
- Transitively delayed: No remaining Architecture Review exposure via Job Cards or Machine Scheduling inputs — AR-004, AR-005, and AR-006 are all Resolved.
...
- Configuration dependencies: Workflow Designer (optional)
- Plugin dependencies: None required; MachineIQ is additive only
- Architecture Review exposure: AR-009 (Quality Check Record naming/scope — see Notes). AR-004 (Machine) exposure is Resolved (Option C, 2026-09-20), AR-005 (Quotation) exposure is Resolved (Option B, 2026-09-26), and AR-006 (Material) exposure is Resolved (Option C, 2026-09-26; ADR-016); none is a convergent input any longer. These are convergent inputs and transitive exposure to end-to-end operation, not direct blockers of the Job Card entity itself.
...
- Convergently blocked: No remaining Architecture Review convergent blocker for tier B (operationally schedulable Job Card) — Material's convergent input (formerly AR-006) is Resolved (Option C, 2026-09-26); Machine's convergent input (formerly AR-004) is Resolved (Option C, 2026-09-20); Quotation's transitive exposure (formerly AR-005) is Resolved (Option B, 2026-09-26). Material allocation's own engineering/implementation work and Machine Scheduling's own engineering completion remain structural prerequisites but are no longer Architecture Review exposures. This is **convergent blocking**, defined in Section 9 — not direct blocking of Job Cards, and not simple transitive delay.
- Transitively delayed: Sales/Estimation's AR-005 exposure on the standard path is Resolved (Option B, 2026-09-26) and no longer a source of transitive delay.
...
- Optional prerequisites: None
- ERPNext dependencies: None — Resolved, Option C (2026-09-20): Machine is a wholly Custom PrintOS DocType
- Configuration dependencies: None required
- Plugin dependencies: MachineIQ (additive only)
- Architecture Review dependencies: None — AR-004 is Resolved; Machine and Machine Profile's structural target is now determined.
- Directly blocked: No — AR-004 is Resolved (Option C, 2026-09-20); Machine and Machine Profile are both Custom PrintOS DocTypes.
...
- Optional prerequisites: None
- ERPNext dependencies: UOM (Native); Item mapping Resolved (AR-006, Option C, 2026-09-26; ADR-016) — Material is a Custom PrintOS master linked to native Item; Substrate is a specialization of Material
- Configuration dependencies: None
- Plugin dependencies: None
- Architecture Review dependencies: None — AR-006 is Resolved; Material/Substrate ownership is settled. Substrate's exact DocType structure (child table vs. standalone) remains deferred to downstream design, but this is not an Architecture Review exposure.
- Directly blocked: No — AR-006 is Resolved (Option C, 2026-09-26). Media Profiles and UOM were never blocked.
- Transitively delayed: Not applicable
- Notes: AR-006's former blast radius extended beyond Inventory itself into Estimation and Purchasing — see Section 14; this is now historical, since AR-006 is Resolved.
...
- Configuration dependencies: None
- Plugin dependencies: None
- Architecture Review dependencies: None remaining — AR-006 (transitive, via Material) is Resolved (Option C, 2026-09-26; ADR-016); AR-007, non-blocking (naming only), remains open.
...
- Transitively delayed: No — Inventory's former AR-006 exposure is Resolved; core Purchase Order/Supplier plumbing was never delayed.
...
- Directly blocked: No
- Transitively delayed: Via Job Cards' general engineering-completion status, not via any remaining Architecture Review chain — AR-004, AR-005, and AR-006 exposure are all Resolved; Job Cards' convergent blocking has cleared.
...
### MachineIQ
- Required prerequisites: Machine (Custom, AR-004 Resolved) and Production Planning/Job Cards data must exist for MachineIQ to have anything to consume
...
- Architecture Review dependencies: None — AR-004 is Resolved; MachineIQ's own scope remains deferred per ADR-008, unrelated to Machine's now-determined base.
...
- Architecture Review dependencies: none — AR-003 is Resolved (registered "AI Assistant" as Proposed only) and is no longer its active dependency; the remaining block comes from the absence of separate future governance covering its architecture, Approved Bounded Context, provider/model decisions, and implementation authorization, none of which AR-003 granted
- Directly blocked: Yes — blocks only its own definitional existence, no implementation owner assigned.
- Transitively delayed: Not applicable
- Notes: Blocks nothing else in the platform. Completely additive if and when separately scoped and governed.
...
Direct blockers: Configuration Studio's Tenant Override (AR-002, Resolved — implementation blocker retained pending Publication and separate implementation authorization, not architectural uncertainty), AI Assistant (its own existence — no covering Architecture Review item; AR-003 is Resolved as a Proposed-name-only registration and does not cover AI Assistant's architecture or existence question). Estimation is no longer a Direct Blocker — Product Template's AR-006 exposure is Resolved (Option C, 2026-09-26; ADR-016); Quotation and Quotation Line remain Pending Architecture Review, not for AR-005's own strategy question, which is Resolved, but because AR-010 remains the open dependency for Cost Estimate's detailed cost-breakdown design and Quotation Line's exact target is deferred to downstream DocType design. Inventory is no longer a Direct Blocker — Material/Substrate's AR-006 exposure is Resolved (Option C, 2026-09-26; ADR-016). Machine Scheduling is no longer a Direct Blocker — AR-004 is Resolved (Option C, 2026-09-20).

Convergently blocked: No module currently has a remaining Architecture Review convergent blocker. Job Cards, tier B (the operationally schedulable Job Card) — its own entity/contract is Custom PrintHub and not individually Pending; Material's convergent input (formerly AR-006) is Resolved (Option C, 2026-09-26; ADR-016), and Machine's convergent input (formerly AR-004) is Resolved (Option C, 2026-09-20). The Job Card work-unit contract (tier A) was never convergently blocked and may be modeled independently. See Section 7 (Job Cards) and Section 8.

Transitive delays only, with no direct blocker or convergent blocking of their own: Production Planning (standard path — its former indirect AR-006 exposure through the Job Cards and Machine Scheduling inputs it consumes is Resolved, as is its indirect AR-004 exposure), Sales (standard path), Artwork (standard path), Dispatch, Accounts (standard path), GST (standard path), Purchasing/Procurement's replenishment logic (though its core Purchase Order/Supplier plumbing is neither blocked nor delayed, and its former AR-006 transitive exposure is also Resolved).
...
| AR-003 | Naming Registry Alignment | Resolved | High | No | AI Assistant (Plugin boundary, Proposed only) | AR-003 naming delay removed; AI Assistant remains excluded pending separate future governance | AR-003 is Resolved: Project Owner selected Option A (2026-09-19). Seven terms (Print Specification, Approval Management, Production Workflow, Machine Management, Finishing, Quality Control, Production Orchestration) are mapped, confirmed, or rejected against existing Approved concepts; none is adopted as a new module. AI Assistant is registered as Proposed only — no Bounded Context, architecture, provider, or model decision, and no implementation authorization; it is not ready, scoped, or implementable. Machine's object-base question (AR-004) is Resolved (Option C, 2026-09-20); the standalone Quality module question (AR-009) remains separately open and unchanged. |
| AR-004 | Machine Domain Ownership | Resolved | High | No | Machine Scheduling, Job Cards (formerly convergent) | Resolved — Machine and Machine Profile both Custom (Option C, 2026-09-20); no longer a direct or convergent blocker | Machine and Machine Profile implementation ownership is settled; Machine Scheduling's own engineering work is unaffected. Job Cards Tier B's formerly remaining AR-006 convergent input is now also Resolved. |
| AR-005 | Quotation Strategy | Resolved | High | No | Estimation (formerly direct), Sales, Production Planning and Job Cards (formerly transitive) | Resolved — Option B, 2026-09-26: Custom Cost Estimate and pricing logic feed ERPNext's native Quotation through a governed handoff; no longer a direct or transitive blocker | Quotation's document strategy is settled; AR-010 remains Open and should be resolved before detailed Cost Estimate cost-breakdown design, to avoid later rework — Quotation Line's exact target and Cost Estimate's cost-breakdown design remain deferred to downstream DocType design. |
| AR-006 | Item vs. Material/Product Template Mapping | Resolved | High | No | Inventory (formerly direct), Estimation (formerly direct, Product Template), Purchasing/Procurement (formerly transitive), Job Cards (formerly convergent) | Resolved — Option C, 2026-09-26 (ADR-016): Material is a Custom PrintOS master linked to native Item; Substrate is a specialization of Material; Product Template is its own Custom PrintOS artifact, independent of Material and Item; no longer a direct, transitive, or convergent blocker | Inventory and Estimation master data alignment is settled; Substrate's exact DocType structure remains deferred to downstream design, but this is not an Architecture Review exposure. |
...
| AR-010 | BOM Necessity for Estimation | Open | Medium | Partial | Estimation | Partial/scoped delay | Affects the Estimation Engine's internal calculation approach, not the existence of core entities. AR-010 remains Open and should be resolved before detailed Cost Estimate cost-breakdown design, to avoid later rework. |
...
- **Job Cards convergence risk (updated).** AR-004 is Resolved (Option C, 2026-09-20), AR-005 is Resolved (Option B, 2026-09-26), and AR-006 is Resolved (Option C, 2026-09-26; ADR-016); Job Cards' convergence point has no remaining Architecture Review dependency (AR-010 continues to affect Estimation's internal cost-breakdown design upstream, but is not itself a Job Cards convergent input). This entry is retained as historical record of the risk that existed while AR-006 remained open.
...
- **AR-006 blast radius (historical).** AR-006 sat underneath Inventory, Estimation, and Purchasing simultaneously while Open; its "High" priority classification in the register, read in isolation, understated how many modules were waiting on it. AR-006 is now Resolved (Option C, 2026-09-26; ADR-016); this entry is retained as historical record.
...
| 1.1 | 2026-09-26 | AR-006 Disposition Synchronization | Synchronized the Matrix with the formal Project Owner disposition of AR-006 (Option C, recorded in `Architecture_Review_Register.md`, Version 0.8; [ADR-016-Item-Material-Product-Template-Mapping](../decisions/ADR-016-Item-Material-Product-Template-Mapping.md)): Material is a Custom PrintOS master record linked to native Item; Substrate is a specialization of Material; Product Template is its own Custom PrintOS artifact, independent of Material and Item. Updated AR-006's official-field reproduction from Open/High/Partial to Resolved/High/No (Section 13), and its Matrix Implementation Impact to record that Inventory and Estimation master data alignment is settled. Corrected Estimation's, Sales's, Production Planning's, Job Cards', Inventory's, Purchasing's, and Dispatch's Architecture Review dependency and blocking-status statements (Section 7) to remove AR-006 as a current direct, transitive, or convergent blocker — Job Cards Tier B now has **no remaining Architecture Review convergent blocker**, since AR-004, AR-005, and AR-006 are all Resolved. Corrected the Architecture Review Dependency definition example (Section 3), the Critical Implementation Path narrative (Section 8), the Direct Blockers/Convergently Blocked/Transitive Delays statements (Section 9), and the Job Cards convergence risk and AR-006 blast-radius entries (Section 14, retained as historical record). No dependency direction, blocker classification (beyond removing AR-006 as a current direct/transitive/convergent blocker), scope classification, or readiness classification was changed beyond recording AR-006 as Resolved. Substrate's exact DocType structure (child table vs. standalone) remains deferred to downstream design and is not specified here; AR-010 remains Open and unchanged. AR-001 through AR-005 remain Resolved as previously recorded; AR-007 through AR-011 remain separately open and unchanged. No implementation authorized. |
| 1.0 | 2026-09-26 | AR-005 Disposition Synchronization | Synchronized the Matrix with the formal Project Owner disposition of AR-005 (Option B, recorded in `Architecture_Review_Register.md`, Version 0.7): Custom Cost Estimate and pricing logic feed ERPNext's native Quotation through a governed handoff — see `../blueprint/14_Quotation_Engine.md`, Version 0.1. Updated AR-005's official-field reproduction from Open/High/Partial to Resolved/High/No (Section 13), and its Matrix Implementation Impact to record that Quotation's document strategy is settled and no longer a direct or transitive blocker, while noting AR-010 remains the open dependency for Cost Estimate's detailed cost-breakdown design and Quotation Line's exact target. Corrected Estimation's Architecture Review dependencies and directly-blocked statement (Section 7) to remove AR-005 as Estimation's own direct blocker while recording that Quotation and Quotation Line remain Pending Architecture Review because of AR-010, not AR-005; corrected Sales's, Production Planning's, and Job Cards' AR-005 exposure and transitive-delay statements (Section 7) to record AR-005 as Resolved; corrected the Architecture Review Dependency definition example (Section 3), the Critical Implementation Path narrative (Section 8), the Direct Blockers statement (Section 9), the Dispatch transitive-delay citation (Section 7), and the Job Cards convergence risk entry (Section 14). Used the normalized wording throughout: AR-010 remains Open and should be resolved before detailed Cost Estimate cost-breakdown design, to avoid later rework. No dependency direction, blocker classification (beyond removing AR-005 as a current direct/transitive blocker), scope classification, or readiness classification was changed beyond recording AR-005 as Resolved. AR-001 through AR-004 remain Resolved as previously recorded; AR-006 through AR-011 remain separately open and unchanged. No implementation authorized. |
| 0.9 | 2026-09-20 | AR-004 Disposition Synchronization | Synchronized the Matrix with the formal Project Owner disposition of AR-004 (Option C, recorded in `Architecture_Review_Register.md`, Version 0.6). Updated AR-004's official-field reproduction from Open/High/Partial to Resolved/High/No, and its Matrix Implementation Impact to record that Machine and Machine Profile are both Custom PrintOS DocTypes and no longer a direct or convergent blocker. Corrected Machine Scheduling's ERPNext-dependency and blocking-status fields (Section 7) from "Undetermined pending AR-004"/"Directly blocked: Yes" to Resolved/"Directly blocked: No"; corrected Job Cards' Architecture Review exposure and convergent-blocking statements (Section 7) to remove Machine/AR-004 as a convergent input, retaining only Material/AR-006; corrected Production Planning's and MachineIQ's indirect AR-004 exposure citations (Section 7); corrected the Architecture Review Dependency definition example (Section 3), the Critical Implementation Path narrative (Section 8), the Direct Blockers and Convergently Blocked statements (Section 9), the Dispatch/Delivery transitive-delay citation (Section 7), the AR-003 row's cross-reference to AR-004 (Section 13), and the Job Cards convergence risk entry (Section 14). No dependency direction, blocker classification (beyond removing AR-004 as a current direct/convergent blocker), scope classification, or readiness classification was changed beyond recording AR-004 as Resolved. AR-001 through AR-003 remain Resolved as previously recorded; AR-005, AR-006, and AR-009 through AR-011 remain separately open and unchanged. No implementation authorized. |
| 0.8 | 2026-09-19 | AR-003 Disposition Synchronization | Synchronized the Matrix with the formal Project Owner disposition of AR-003 (Option A, recorded in `Architecture_Review_Register.md`, Version 0.5). Updated AR-003's official-field reproduction from Open/High/Partial to Resolved/High/No, and its Matrix Implementation Impact to "AR-003 naming delay removed; AI Assistant remains excluded pending separate future governance." Corrected additional active drift: AI Assistant's foundational-dependency row (Section 5) and detailed dependency block (Section 9) updated from "unregistered, unscoped" / "Architecture Review dependencies: AR-003" to "Proposed name only, no Bounded Context, unscoped" with no active AR-003 dependency, since AR-003 is Resolved and its remaining block comes from the absence of separate future governance covering its architecture, Approved Bounded Context, provider/model decisions, and implementation authorization. Corrected the Artwork module's Architecture Review dependency line and the Direct Blockers summary (Section 10) to state that Artwork itself is not governed by AR-003 and that Approval Record's own naming remains a separate, currently unassigned governance question, distinct from the "Approval Management" module-name mapping AR-003 resolved. No dependency direction, blocker classification, or readiness classification changed beyond recording AR-003 as Resolved and correcting these misattributions. AR-004 and AR-009 remain separately open and unchanged; AI Assistant is not represented as ready, scoped, or implementable and remains excluded pending separate future governance. No implementation authorized. |
