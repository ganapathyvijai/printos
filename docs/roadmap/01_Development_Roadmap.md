...
- **Job Card Tier B (the operationally schedulable Job Card) is the principal convergence point** of the dependency graph, where AR-006's consequence remains (AR-004's Machine-Scheduling consequence Resolved, 2026-09-20; AR-005's Quotation-strategy consequence Resolved, 2026-09-26) (Matrix §8; Section 15).
...
| Estimation | Estimation | Conditional Roadmap Scope | Architecture Review Blocked | AR-006, AR-010 (AR-005 Resolved, Option B, 2026-09-26); AR-001 resolved, v16 technical revalidation pending | Define Conditional Workstream | Architecture Review Blocked | Not Authorized | "Key differentiator"; directly blocked on Product Template (AR-006); Quotation document strategy is settled but Quotation Line's exact target and Cost Estimate's cost-breakdown design remain deferred pending AR-010 |
| Sales | Core Business Operations | Conditional Roadmap Scope | Conditional Architecture | AR-001 resolved, v16 technical revalidation pending; Estimation's remaining AR-006/AR-010 exposure (AR-005 Resolved, Option B, 2026-09-26, no longer a source of transitive delay) | Define Conditional Workstream | Conditional | Not Authorized | Reorder path independent of Estimation standard path |
...
| Production Planning | Production Operations | Conditional Roadmap Scope | Conditional Architecture | AR-006 (transitive/convergent via inputs; AR-004 exposure Resolved; AR-005 exposure Resolved, Option B, 2026-09-26) | Define Conditional Workstream | Conditional | Not Authorized | Distinct module (ADR-014); consumes, does not create Job Card |
| Job Cards | Production Operations | Conditional Roadmap Scope (Tier A / Tier B gates) | Conditional Architecture | AR-006 (convergent, Tier B), AR-009; AR-004 convergent exposure Resolved; AR-005 transitive exposure Resolved (Option B, 2026-09-26) | Define Conditional Workstream | Conceptually Ready (Tier A) / Conditional (Tier B) | Not Authorized | One aggregate, one DocType; tiers are readiness gates |
...
- **Conditional content:** Quotation Line's exact target and Product Template ownership; Estimation Engine internal cost-breakdown approach. Quotation's own document strategy is Resolved (AR-005, Option B, 2026-09-26): Custom Cost Estimate and pricing logic feed ERPNext's native Quotation through a governed handoff.
- **Direct AR gates:** AR-006 (Material/Product Template). AR-005 (Quotation strategy) is Resolved and no longer a direct gate.
...
- **Permitted Working Assumptions:** Estimation → Sales boundary as frozen. ERPNext v16 is no longer an assumption — it is the AR-001-governed target (reaffirming Accepted ADR-001); capability-level Estimation claims remain conditional pending technical revalidation. Quotation's document strategy (Option B) is likewise no longer an assumption — it is the AR-005-governed disposition.
- **Prohibited finalization:** Quotation Line's exact target and downstream handoff mechanics; Material/Product Template mapping; whether BOM is adopted.
- **Traceability:** `blueprint/05_Domain_Model.md`; `blueprint/06_Bounded_Contexts.md`; `blueprint/14_Quotation_Engine.md`; AR-005 (Resolved)/006/010; ADR-013; `ERPNext_DocType_Mapping.md` (Pending rows); Matrix §7.
- **Implementation authorization:** Not Authorized.
...
- **Conditional content:** standard-path completion transitively delayed via Estimation's remaining AR-006/AR-010 exposure and the Job Cards convergence (Estimation's AR-005 Quotation-strategy exposure is Resolved, Option B, 2026-09-26, and no longer a source of delay).
- **Direct AR gates:** none of their own. **Transitive:** Estimation's remaining AR-006/AR-010 exposure (Sales/Artwork standard path); via Job Cards for Dispatch. **Naming:** AR-008 (Dispatch/Delivery, non-blocking).
...
- **Traceability:** `blueprint/06_Bounded_Contexts.md`; `blueprint/10_Business_Workflows.md`; AR-005 (Resolved)/008; Matrix §7, §9.
- **Implementation authorization:** Not Authorized.
...
- **Convergent gates:** AR-006 converges on Job Cards Tier B (AR-004's convergent contribution Resolved).
...
- **Prohibited finalization:** operational (Tier B) Job Card design; anything requiring the AR-006 convergent input. Machine ownership design is no longer prohibited at the category level (Resolved, Option C) but DocType-level design and implementation remain separately unauthorized pending Blueprint 16 review and scoped implementation authorization.
- **Implementation authorization:** Not Authorized.
...
- Remains **Conditional** — the convergence point where **AR-006** (Material, via allocation) remains; AR-004's Machine/Machine Scheduling contribution is Resolved (Option C, 2026-09-20) and AR-005's Quotation, transitive via Sales Order, contribution is Resolved (Option B, 2026-09-26). This is **convergent blocking**, not direct blocking of the Job Card entity.
...
**Traceability:** ADR-014; `blueprint/09_PrintOS_Modules.md`; `Canonical_Domain_Model.md`; `ERPNext_DocType_Mapping.md` (single Job Card row); AR-004 (Resolved)/005 (Resolved)/006/009; Matrix §6–9.
