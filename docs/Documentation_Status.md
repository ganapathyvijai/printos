...
| Roadmap | 1 | 01_Development_Roadmap (Approval, Version 1.1; Architecture Review, Business Review, and Project Owner Approval complete); `00_` index slot reserved for a future Roadmap index |
...
| **Approval** | 2 | `docs/implementation/Architecture_Freeze.md` (Version 1.1) and `docs/roadmap/01_Development_Roadmap.md` (Version 1.1) — following formal Project Owner approval (2026-07-26) of, respectively, the Layered Architecture Freeze and the PrintHub Development Roadmap (Architecture Review, Business Review, and Project Owner Approval all complete for the roadmap), each subsequently updated by a factual AR-001 disposition synchronization (Freeze 1.0→1.1; Roadmap 1.0→1.1) that changed no frozen scope, roadmap scope, or readiness classification. Reaching Approval records that each document has an effective, approved disposition within its declared scope; it does not mean either document is Published, does not declare a Full Architecture Freeze, and does not authorize implementation. Increased by one from the prior count (1) as the Development Roadmap moved from Draft to Approval. |
| **Review** | 0 | No document has a recorded Review-stage `Status`. |
| **Draft** | 106 | Documents carrying a `Status: Draft` (or `Draft`-prefixed) header — Blueprint, Business, Technical, Database, Configuration, Architecture, Standards, populated Implementation (other than `Architecture_Freeze.md`, now Approval), and the two former-Published indexes (`blueprint/00_Master_Index.md`, `decisions/00_ADR_Index.md`). Decreased by one from the prior count (107) as `roadmap/01_Development_Roadmap.md` moved from Draft to Approval. |
...
4. **Empty reserved categories** — `docs/api/`, `docs/sprints/`, `docs/ui/`, `docs/changelog/`, `docs/prompts/` exist but are empty. (`docs/roadmap/` is populated with the approved Development Roadmap, Approval, Version 1.1, and is no longer empty.)
