...
- **Architecture Review Dependency** — A module is blocked or delayed by an open Architecture Review Register item. Example: AR-006 Material/Product Template, AR-010 BOM necessity for Cost Estimate's detailed cost-breakdown design (AR-004 Machine and AR-005 Quotation Strategy were examples of this category until their Resolution, 2026-09-20 and 2026-09-26 respectively).
...
- Configuration dependencies: None required to bootstrap
- Plugin dependencies: None
- Architecture Review dependencies: AR-005 (Quotation strategy) is Resolved — Option B, 2026-09-26: Custom Cost Estimate and pricing logic feed ERPNext's native Quotation through a governed handoff; AR-006 (Material/Product Template, shared with Inventory) remains open; AR-010 (BOM necessity — affects the Estimation Engine's internals, not entity existence) remains Open and should be resolved before detailed Cost Estimate cost-breakdown design, to avoid later rework
- Directly blocked: Yes — Product Template is Pending Architecture Review (AR-006). Quotation and Quotation Line remain Pending Architecture Review as well, not because AR-005 is unresolved (it is Resolved), but because AR-010 remains the open dependency for Cost Estimate's detailed cost-breakdown design and Quotation Line's exact target is deferred to downstream DocType design. Job Types, Finishing Types, and Paper Sizes reference data are not blocked.
...
- Plugin dependencies: None
- Architecture Review dependencies: AR-005 is Resolved (Option B, 2026-09-26); Estimation's remaining AR-006 and AR-010 exposure reaches Sales only indirectly, for the standard Quotation-based path
- Directly blocked: No — Sales Order itself is Extended ERPNext, not Pending.
- Transitively delayed: Yes, for the standard path, via Estimation's remaining AR-006/AR-010 exposure.
...
- Architecture Review dependencies: AR-005 (Quotation) is Resolved (Option B, 2026-09-26) and no longer an exposure via the Sales Order standard path; AR-006 reaches Production Planning indirectly, through the Job Cards and Machine Scheduling inputs it consumes (AR-004 exposure removed — Resolved, Option C, 2026-09-20)
- Directly blocked: No — Production Planning as a capability has no Architecture Review item of its own.
- Transitively delayed: Yes — via its consumption of Job Cards and Machine Scheduling (which carry AR-006 exposure); AR-005 exposure on the Sales/Estimation standard path is Resolved and no longer a source of delay.
...
- Configuration dependencies: Workflow Designer (optional)
- Plugin dependencies: None required; MachineIQ is additive only
- Architecture Review exposure: AR-006 (Material — convergent input, via allocation to tier B), AR-009 (Quality Check Record naming/scope — see Notes). AR-004 (Machine) exposure is Resolved (Option C, 2026-09-20) and AR-005 (Quotation) exposure is Resolved (Option B, 2026-09-26); neither is a convergent input any longer. These are convergent inputs and transitive exposure to end-to-end operation, not direct blockers of the Job Card entity itself.
...
- Convergently blocked: Yes, for tier B (operationally schedulable Job Card) — end-to-end operation cannot complete without the Material (AR-006) convergent input, blocked at its own source. Machine's convergent input (formerly AR-004) is Resolved (Option C, 2026-09-20) and no longer blocked; Machine Scheduling's own engineering completion remains a structural prerequisite but is no longer an Architecture Review exposure. This is **convergent blocking**, defined in Section 9 — not direct blocking of Job Cards, and not simple transitive delay.
...
- Directly blocked: No
- Transitively delayed: Yes — via Job Cards' convergent blocking (AR-006 chain; AR-004 and AR-005 exposure both Resolved).
...
Direct blockers: Estimation (Product Template — AR-006; Quotation and Quotation Line remain Pending Architecture Review as well, not for AR-005's own strategy question, which is Resolved, but because AR-010 remains the open dependency for Cost Estimate's detailed cost-breakdown design and Quotation Line's exact target is deferred to downstream DocType design), Inventory (Material/Substrate — AR-006), Configuration Studio's Tenant Override (AR-002, Resolved — implementation blocker retained pending Publication and separate implementation authorization, not architectural uncertainty), AI Assistant (its own existence — no covering Architecture Review item; AR-003 is Resolved as a Proposed-name-only registration and does not cover AI Assistant's architecture or existence question). Machine Scheduling is no longer a Direct Blocker — AR-004 is Resolved (Option C, 2026-09-20).

Convergently blocked: Job Cards, tier B (the operationally schedulable Job Card) — its own entity/contract is Custom PrintHub and not individually Pending, but operational completion structurally requires Material (AR-006) as a convergent input, directly blocked at its own source. Machine's convergent input (formerly AR-004) is Resolved (Option C, 2026-09-20). The Job Card work-unit contract (tier A) is not convergently blocked and may be modeled independently. See Section 7 (Job Cards) and Section 8.

Transitive delays only, with no direct blocker or convergent blocking of their own: Production Planning (standard path — though it also carries indirect AR-006 exposure through the Job Cards and Machine Scheduling inputs it consumes; its indirect AR-004 exposure is Resolved), Sales (standard path), Artwork (standard path), Dispatch, Accounts (standard path), GST (standard path), Purchasing/Procurement's replenishment logic (though its core Purchase Order/Supplier plumbing is neither blocked nor delayed).
...
- **Job Cards convergence risk (updated).** AR-004 is Resolved (Option C, 2026-09-20) and AR-005 is Resolved (Option B, 2026-09-26); Job Cards' convergence point now depends on AR-006 alone (with AR-010 continuing to affect Estimation's internal cost-breakdown design upstream). AR-006 resolving late continues to have an outsized effect on the entire Production capability (Production Planning, Job Cards, and Machine Scheduling together), disproportionate to what a simple count of "one AR item per entity" would suggest.
