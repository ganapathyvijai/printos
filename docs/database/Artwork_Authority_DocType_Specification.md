...
- [../blueprint/18_Artwork_Management.md](../blueprint/18_Artwork_Management.md) — currently **Approval, Version 1.6** (2026-09-13; promoted in place, at the same version number, since Version 1.6 remained uncommitted — `ART-M4-LCV-F1` corrected), promoting to Approval the Model 4 seventeen-rule package (Sections 2.1/2.2) and its five-finding correction, all with no content change from the prior Draft state of Version 1.6; **Approval, Version 1.3** (Granted 2026-08-19 at Version 1.1, following completed Architecture Review — Accepted with non-blocking corrections — and Business Review — Accepted with non-blocking corrections — of the Version 1.1 package, with all six local Artwork findings Closed; carried forward through a bounded reference-only correction and a bounded naming-status synchronization, both on 2026-08-22) is the immediately prior approved baseline; prior historical baselines before that **Approval, Version 1.2**, **Approval, Version 1.1**, and **Approval, Version 1.0** (Granted 2026-07-31). This is the governing architecture/design authority for this specification. This Approval is **not** Publication of this DocType Specification, **not** runtime-validation authorization, and **not** implementation authorization.
...
- [../implementation/JobCard_TierA_System_Design.md](../implementation/JobCard_TierA_System_Design.md) — the consuming Production design; currently **Approval, Version 1.11** (2026-09-20, a reference-only AR-003 disposition synchronization from the committed Approval, Version 1.10 baseline), with **Approval, Version 1.10** (2026-09-13, promoted in place at the same version number since it remained uncommitted at that time, adopting Rule 11/Section 10.5 and its three-finding correction, with no content change) preserved as historical record, **Approval, Version 1.7** (Project Owner Document Lifecycle Approval granted 2026-08-19 at Version 1.5; carried forward through its own bounded reference-only correction and naming-status synchronization, both on 2026-08-22) as the immediately prior approved baseline before that, and prior historical baselines before that **Approval, Version 1.6**, **Approval, Version 1.5**, and **Approval, Version 1.4**. Its Version 1.5 content records the `production_artwork_set` reference contract, now approved. (`ART-M4-LCV-F2` corrected.)
...
No claim is made that these names are Approved by the Naming Registry. This specification does not modify the Naming Registry beyond citing its Section 13a Proposed entries. **`AR-003` was separately Resolved by Project Owner decision on 2026-09-19** ([Architecture Review Register](../decisions/Architecture_Review_Register.md), Version 0.5; [Naming Registry](../standards/Naming_Registry.md) Section 40) — that resolution concerns only its own eight named terms (none of which is any of the five technical names above) and was **not** performed by, and does **not** substantively modify, this specification. All five technical names above remain **Under Review**; **none is Approved**. This document's Status remains **Draft**; it is **not Published** and remains **not safe for coding**.
...
## 24.1 Artwork Applicability Design Direction — Model 4 (Project Owner decision 2026-09-12)

Per explicit Project Owner decision dated 2026-09-12, recorded as **A4, B2, C2**, the adopted high-level design direction is **Tenant enablement with governed per-order applicability**. The governing statement is `../blueprint/18_Artwork_Management.md` Section 2.1. This section records only what that direction means for **this specification**.
...
**What remains governed and unchanged.** Every DocType, field, state, guard, uniqueness rule, revocation-propagation rule, and audit requirement already specified in this document continues to apply in full to work classified as **Artwork-controlled**. Artwork Authority remains shared governed functionality within `printos_core`; "not applicable" is not an informal waiver, manual override, or permission bypass; applicability history must not be silently rewritten after it has governed production authority; and **no existing record is automatically migrated, rebound, or reclassified**.

**Status as of Version 1.5 — policy adopted, no specification content added.** Per explicit Project Owner decision dated 2026-09-12 ("Adopt all seventeen recommended selections exactly as listed in the verified Model 4 ratification sheet. Exceptions: None."), the seventeen items formerly listed here as unresolved now each carry an adopted business and architecture policy, recorded normatively in `../blueprint/18_Artwork_Management.md` Section 2.2 and, for the non-Artwork release authority (Rule 11), in `../implementation/JobCard_TierA_System_Design.md` Section 10.5. **This specification is not the authoritative source for any of the seventeen rules and does not restate them normatively.** Adoption of a policy is **not** a field, schema, API, permission, workflow, or migration definition — the three "No specification content is approved or added by this decision" bullets above remain fully in force, and each rule's own intentionally deferred execution-level detail remains unresolved.

**Lifecycle effect.** This specification remains **Draft**, **not Published**, and **not safe for coding**. Targeted Architecture Review and Business Review of the seventeen-rule policy are **Pending**, and it is not safe for coding until its remaining execution-level rules are governed, reviewed, and independently verified. No pre-Publication gate in Section 22 is closed by this decision, and all 62 Artwork runtime gates and the Artwork production gate remain **Open**.
...
- **Governing Artwork System Design:** `../blueprint/18_Artwork_Management.md` — currently **Approval, Version 1.6** (2026-09-13; promoted in place, at the same version number, since Version 1.6 remained uncommitted — `ART-M4-LCV-F1` corrected), promoting to Approval the Model 4 seventeen-rule package (Sections 2.1/2.2) and its five-finding correction, all with no content change from the prior Draft state of Version 1.6; **Approval, Version 1.3**, Project Owner Document Lifecycle Approval **Granted 2026-08-19** at Version 1.1 (Architecture Review and Business Review of the Version 1.1 package both Accepted with non-blocking corrections; all six local Artwork findings Closed), carried forward through a bounded reference-only correction and a bounded naming-status synchronization, both on **2026-08-22**, is the immediately prior approved baseline; prior historical baselines before that **Approval, Version 1.2**, **Approval, Version 1.1**, and **Approval, Version 1.0**, Granted 2026-07-31. This governing approval is **not** Publication of this DocType Specification and **not** implementation authorization.
...
|---|---|---|---|
| 1.8 | 2026-09-20 | AR-003 Disposition Synchronization | Corrected the active Section 4 (Naming Boundary) closing sentence to record that `AR-003` was **separately Resolved** by Project Owner decision on **2026-09-19** ([Architecture Review Register](../decisions/Architecture_Review_Register.md), Version 0.5; [Naming Registry](../standards/Naming_Registry.md) Section 40) — a resolution concerning only AR-003's own eight named terms, none of which is any of this specification's five technical names, which this specification **did not perform** and **is not substantively modified by**. All five technical names remain **Under Review**; **none is Approved**. This specification's own Status remains **Draft**; it is **not Published** and remains **not safe for coding**. Synchronized the Section 3 citation of `../implementation/JobCard_TierA_System_Design.md` to its current **Approval, Version 1.11** (2026-09-20, reference-only AR-003 disposition synchronization from the committed Approval, Version 1.10 baseline), preserving Approval, Version 1.10 as historical record and Approval, Version 1.7 as the immediately prior approved baseline before that. **No conceptual DocType, field, state model, constraint, controlled operation, permission, test, migration rule, or runtime-validation gate was modified; no technical name was selected or Approved; no directory, Module Def, or file was created.** All 62 Artwork runtime gates and the Artwork production gate remain **Open**. No implementation, Publication, or runtime-validation authority is granted. |
| 1.7 | 2026-09-13 | Project Owner Lifecycle Decision — Citation Synchronization (Option A); `ART-M4-LCV-F1` Consolidation | Per explicit Project Owner decision dated 2026-09-13 (Option A of the Model 4 Lifecycle Decision Brief, exceptions none), synchronizes the two current-state citations of the governing Artwork System Design to **Approval, Version 1.6** (2026-09-13; promoted in place, at the same version number, since Version 1.6 remained uncommitted), which promotes to Approval the Model 4 seventeen-rule package and its five-finding correction with no content change from the prior Draft state of Version 1.6; **Approval, Version 1.3** is correctly re-identified as the *prior* approved baseline (no longer "the last"), preserved unchanged. This citation was briefly, and erroneously, synchronized to a "Version 1.7" that the governing System Design never retained (`ART-M4-LCV-F1`); it is corrected here to Version 1.6, pending separate independent closure verification of `ART-M4-LCV-F1`. **This specification's own version/status are unaffected: it remains Draft, Version 1.7, not safe for coding** — the Project Owner's decision explicitly retains it as Draft. This specification is not the authoritative source for the seventeen-rule package and does not restate it normatively. **Review and finding treatment.** The System Design's Architecture Review, Business Review, and Documentation Governance dispositions are now accepted at the System Design level with all five corrections independently verified Closed; this citation update does not itself accept, complete, or finalize anything, and does not resolve any of the twelve findings from the separate, still-pending six-name naming review. No AR identifier was created; `AR-003` was not modified. **Scope.** This specification remains **Draft**, **not Published**, and **not safe for coding**; its five technical names remain **Under Review**, none Approved; `ART-RVR-C08` remains **incomplete**. All 62 Artwork runtime gates and the Artwork production gate remain **Open**. No implementation, Publication, or runtime-validation authority is granted. **`ART-M4-LCV-F2` (2026-09-13, same day, before commit).** This specification's own Related Documents citation of the Job Card Tier A System Design was found stale at "currently Approval, Version 1.7," never having been advanced through that document's own 1.8–1.10 arc; corrected to cite Approval, Version 1.10, with Approval, Version 1.7 retained as its immediately prior approved baseline. `ART-M4-LCV-F2` is corrected, pending separate independent closure verification, and is not Closed by this entry. |
| 1.6 | 2026-09-12 | Project Owner Decision — Four-Finding Correction (Citation Synchronization) | Per explicit Project Owner decision dated 2026-09-12 (Option A of the Project Owner Decision Brief, exceptions none), synchronizes the two current-state citations of the governing Artwork System Design to **Draft, Version 1.6** (amending Section 2.2 Rules 2, 9, 16 to close `ART-M4-17-ARCH-F1`, `ART-M4-17-ARCH-F2`, `ART-M4-17-ARCH-F3`, `ART-M4-17-BIZ-F1`), with **Approval, Version 1.3** unchanged as the preserved baseline. **This specification is explicitly not the authoritative source for any of the amended rules and does not restate them normatively** — the three "No specification content is approved or added by this decision" bullets in Section 24.1 remain fully in force. **This is citation synchronization only.** Workflow Section 8 confirms **MINOR**. **Review and finding treatment.** The four findings are corrected in the cited authority, pending separate independent closure verification; **none is Closed by this entry**. No AR identifier was created; `AR-003` was not modified. **Scope.** This specification remains **Draft**, **not Published**, and **not safe for coding**. All 62 Artwork runtime gates and the Artwork production gate remain **Open**. |
| 1.5 | 2026-09-12 | Project Owner Decision — Model 4 Seventeen-Rule Definition Adoption (Citation Synchronization) | Per explicit Project Owner decision dated 2026-09-12 — "Adopt all seventeen recommended selections exactly as listed in the verified Model 4 ratification sheet. Exceptions: None." — updates Section 24.1's "Unresolved and not supplied here" paragraph to state that all seventeen items now carry an adopted policy, recorded normatively in `../blueprint/18_Artwork_Management.md` Section 2.2 and, for Rule 11 (non-Artwork release authority), in `../implementation/JobCard_TierA_System_Design.md` Section 10.5. **This specification is explicitly not the authoritative source for any of the seventeen rules and does not restate them normatively** — it is a derived specification, and the three "No specification content is approved or added by this decision" bullets in Section 24.1 remain fully in force. **This is citation synchronization, not new normative content of its own**, but because the cited authorities (Artwork SD Section 2.2, Job Card SD Section 10.5) are themselves un-reviewed new content, this specification's own review posture is unaffected — it was already Draft and remains Draft. Workflow Section 8 confirms **MINOR** as the correct increment. **The seventeen items' own intentionally deferred execution-level detail remains unresolved**; nothing here supplies a field, fieldtype, Link target, default, option set, index, API, permission rule, import rule, validation behavior, DocType, child table, role, workflow transition, or migration mechanism. **Review and finding treatment.** Both review lenses remain **Pending**; **no finding is Closed**; `ART-NBR-F5` is answered at direction and policy level only in the cited authorities, not resolved here. No AR identifier was created; `AR-003` was not modified. **Scope.** This specification remains **Draft**, **not Published**, and **not safe for coding**. No pre-Publication gate in Section 22 is closed. All 62 Artwork runtime gates and the Artwork production gate remain **Open**. |
| 1.4 | 2026-09-12 | Project Owner Decision — Model 4 Artwork Applicability Design Direction (A4, B2, C2) | Per explicit Project Owner decision dated 2026-09-12, adds new Section 24.1 recording the adopted **Model 4** design direction — Tenant enablement with governed per-order applicability — and its consequences for this specification, and synchronizes the two current-state citations of the governing Artwork System Design (Sections 2 and 25) to **Draft, Version 1.4**, with **Approval, Version 1.3** recorded as the last fully reviewed and Owner-approved baseline. **Explicitly recorded:** no field or schema definition has been approved for enablement or applicability; no server transition, API, permission rule, import rule or validation behavior has been defined; and no DocType, child table, role, workflow transition, migration or runtime rule is created. Every existing DocType, field, state, guard, uniqueness, revocation-propagation and audit requirement continues to apply in full to Artwork-controlled work. Seventeen rule-definition requirements remain **unresolved**, with no answer supplied to any of them; "not applicable" is not an informal waiver, manual override or permission bypass; applicability history must not be silently rewritten after governing production authority; and **no existing record is automatically migrated, rebound or reclassified**. **Lifecycle.** Additive, non-contradictory expansion — **MINOR** increment under `../Documentation_Workflow.md` Section 8. This specification remains **Draft**, **not Published**, and **not safe for coding**; targeted Architecture Review and Business Review of the Model 4 direction are **Pending**. **Review and finding treatment.** Conditional acceptance of an advisory disposition **does not complete a governed review lens**: Architecture Review and Business Review of the six technical names both remain **Pending**, and all twelve independently verified findings (`ART-NAR-F1`–`F5`, `ART-NBR-F1`–`F5`, `ART-TLV-F1`–`F2`) remain **pending — none is Closed**. These are local review labels, **not** Architecture Review Register identifiers; **no AR identifier was created and `AR-003` was not modified or resolved**. All five Artwork technical names remain **Under Review**; none is Approved. `ART-RVR-C08` remains incomplete; `ART-RVR-B02` remains stable and design/governance-resolved. No Section 22 pre-Publication gate is closed; all 62 Artwork runtime gates and the Artwork production gate remain **Open**; no Full Architecture Freeze is claimed; Publication, implementation, deployment and runtime-validation authority are **not** granted. |
