...
| ART-RVR-C08 | Source-based collision verification of the six proposed technical names (`PrintHub Artwork`, `PrintHub Artwork Revision`, `PrintHub Customer Approval Evidence`, `PrintHub Production Artwork Set`, `PrintHub Production Artwork Set Item`, `PrintHub Job Card`) against the governed Frappe, ERPNext, `printos_core` (when available), and specifically governed plugin population — **governance scope decided 2026-08-25** (`../standards/Naming_Registry.md` Section 13b), **inspection itself not yet performed** | Naming Registry §39 checklist ("ERPNext compatibility confirmed — no collision with ERPNext Reserved Terms, Section 21"); Naming Registry §13b (governance direction) | Partially checked from documentation only: none of the six names appears in Naming Registry §21's curated Reserved Terms table (a small, explicitly non-exhaustive list). **Exact Frappe/ERPNext/`printos_core`/plugin source identities remain unselected**; required source roots remain missing or unconfirmed; `printos_core`'s absence is recorded as not-yet-available, not as inspected or collision-free; source acquisition remains separately unauthorized; static inspection itself remains unexecuted; no runtime evidence exists | Full Frappe/ERPNext/`printos_core`/plugin DocType and module namespace verification, against separately selected exact source identities — not obtainable from documentation alone | Confident Approval of the six names remains unsupported until this evidence exists; **names entered Proposed status on 2026-08-22**; the collision-evidence governance scope was decided 2026-08-25, but source identities, acquisition, and inspection all remain separately unauthorized and unperformed; **static inspection alone, once performed, will still not constitute final Tenant-specific collision assurance** — a separately governed later deployment-time check against each Tenant's actual app inventory is additionally required before installation, migration, or activation | (1) Separately select or confirm exact Frappe/ERPNext/`printos_core`/plugin source identities; (2) a separate, explicitly bounded source-inspection or source-acquisition authorization; (3) authorized static inspection; (4) independent verification of that inspection's evidence; (5) a later, separately governed Tenant-specific deployment-time collision check |
