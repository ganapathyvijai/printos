...
| Artwork | Sales (confirmed Sales Order) |
| Job Cards | Sales Order, Approved Artwork (work-unit contract, tier A); Machine Scheduling + Inventory + Production Planning output for operational completion (tier B) |
| Machine Scheduling | Job Card work-unit contract; Machine Profile |
| Production Planning | Sales, Artwork; consumes Job Cards + Machine Scheduling → produces Scheduled Job Cards |
...
**Matrix Implementation Impact — AR-001:** All ERPNext-native and Extended classifications in this layer, and throughout this matrix, are stated against a *documented working assumption* — not a resolved architecture decision — that the ERPNext platform version is v16, the basis already used in [../architecture/ERPNext_Fit_Analysis.md](../architecture/ERPNext_Fit_Analysis.md) (Published Blueprint/[ADR-001-ERPNext-Framework](../decisions/ADR-001-ERPNext-Framework.md)). AR-001 remains **Critical and development-blocking** in the Architecture Review Register. Foundation modules carry no *module-specific* Architecture Review exposure, but they remain exposed to the *cross-cutting* AR-001 version question: their ERPNext-facing classifications are conditionally assessable only — dependency-light but version-sensitive. This matrix does not authorize ERPNext version-sensitive implementation ahead of AR-001's disposition; final implementation of any ERPNext-native or Extended element should await AR-001 resolution wherever ERPNext version behavior materially affects it. See Section 13 for AR-001's official register status.
...
- Transitively delayed: No
- Notes: Customer carries no module-specific Architecture Review blocker beyond the cross-cutting AR-001 version assumption (Section 5); only Enquiry-specific behavior is gated by AR-011.
...
### Artwork
- Required prerequisites: Sales (confirmed Sales Order)
...
- Notes: Approved Artwork is a hard gate before Job Cards production may begin, per Business Rules.
...
- Directly blocked: No — Purchase Order and Supplier are Native ERPNext with no module-specific Architecture Review exposure; they remain subject to the cross-cutting AR-001 version assumption (Section 5).
...
- Architecture Review dependencies: AR-002, for Tenant Override specifically
- Directly blocked: Tenant Override only. All other Configuration Studio artifacts are unblocked.
- Transitively delayed: No
- Notes: Feature Flag, Automation Rule, and Module Definition have no native ERPNext fallback and are true hard prerequisites for any module that needs conditional feature-gating, safe automation, or module enable/disable — see Section 11 for the full decomposition.
...
- Architecture Review dependencies: AR-002, for tenant-scoped plugin configuration only
- Directly blocked: No
- Transitively delayed: No
- Notes: No plugin category is a Phase 1 blocker; every category has a documented degrade-gracefully or manual-fallback path.
...
Direct blockers: Estimation (Quotation, Product Template — AR-005, AR-006), Inventory (Material/Substrate — AR-006), Machine Scheduling (Machine, Machine Profile — AR-004), Configuration Studio's Tenant Override (AR-002), AI Assistant (its own existence — AR-003).
...
**Runtime consumption (Tenant-scoped, Pending AR-002):** Tenant Override specifically — and only the *tenant-aware* version of any configuration artifact — is gated by AR-002. The single-installation, default-configuration version of every artifact in the second group above is unaffected by AR-002's resolution status.
...
|---|---|---|---|---|---|---|---|
| AR-001 | ERPNext Platform Version (15/16 conflict) | Open | Critical | Yes | Cross-cutting — every ERPNext-facing Native/Extended classification in this matrix | Direct blocker (cross-cutting) | ERPNext-native and Extended classifications throughout this matrix rest on a documented working assumption of ERPNext v16 (Published Blueprint/ADR-001), not a resolved decision — see Section 5. This matrix does not authorize version-sensitive implementation ahead of AR-001's disposition; every ERPNext-dependency entry in Section 7 is conditionally assessable only and must be re-verified once AR-001 is resolved. |
| AR-002 | Multi-Tenant Strategy Alignment | Open | Critical | Yes | Configuration Studio (Tenant Override), Plugin Architecture (tenant-scoped config) | Partial/scoped delay | The register's official Blocks Development field is Yes; this matrix's practical interpretation is that AR-002 delays only the tenant-aware layer of configuration (Tenant Override) and does not delay single-installation, default-configuration implementation of any module. This interpretation does not alter the register's own Yes/Critical classification. |
...
- **AR-001 version-basis risk.** Every Native/Extended ERPNext classification in this matrix rests on the version assumption stated in Section 5. If AR-001 resolves to a different version than assumed, foundation-layer and ERPNext-dependency entries throughout Section 7 may need re-verification, not just Estimation/Production-adjacent ones.
