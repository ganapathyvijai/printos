...
| **Total** | **190** | Live count of tracked `docs/**/*.md` files (186 across the categories above + 4 root-level: README, Documentation_Map, Documentation_Status, Documentation_Workflow). Excludes 5 empty reserved folders (`api`, `changelog`, `prompts`, `sprints`, `ui`); includes 17 empty Placeholder files (blueprint 11–20 and implementation 09–15). Increased by 2 from the prior count (188) due to two new documents: `decisions/ADR-015-Tenant-Company-Multi-Tenancy-Model.md` and `blueprint/25_MultiTenant_Architecture.md`. |
...
| **Draft** | 106 | Documents carrying a `Status: Draft` (or `Draft`-prefixed) header — Blueprint, Business, Technical, Database, Configuration, Architecture, Standards, populated Implementation (other than `Architecture_Freeze.md`, now Approval), and the two former-Published indexes (`blueprint/00_Master_Index.md`, `decisions/00_ADR_Index.md`). Unchanged from the prior count (106); the new documents this period landed in Accepted (ADR-015) and Approval (`25_MultiTenant_Architecture.md`), not Draft. |
| **Accepted** (ADR-specific lifecycle) | 15 | All of `docs/decisions/ADR-001` through `ADR-015`; the only category with binding, in-force status today. Increased by one from the prior count (14) with the addition of ADR-015-Tenant-Company-Multi-Tenancy-Model (Accepted, Version 1.0), the resolving decision for AR-002 Option A. |
...
| **Total tracked Markdown documents** | 190 | Live count of tracked `docs/**/*.md` files; increased by 2 from the prior count (188) with the addition of `decisions/ADR-015-Tenant-Company-Multi-Tenancy-Model.md` (Accepted) and `blueprint/25_MultiTenant_Architecture.md` (Approval). (Separately, 5 reserved folders remain empty with no tracked files: `api`, `changelog`, `prompts`, `sprints`, `ui`; `roadmap` remains populated.) |
