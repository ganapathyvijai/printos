...
#### Reuse Opportunity
High for Warehouse (exact match, per [../implementation/03_ERPNext_Mapping.md](../implementation/03_ERPNext_Mapping.md)) and stock ledger mechanics. Item remains the native record for inventory, purchasing, unit-of-measure, and accounting integration; Material/Substrate are Resolved as Custom PrintOS records — see Gap Analysis.

#### Gap Analysis
Naming Registry Section 27, item 10 (ERPNext "Item" vs. PrintOS "Material"/"Product Template") is **Resolved** — [ADR-016-Item-Material-Product-Template-Mapping](../decisions/ADR-016-Item-Material-Product-Template-Mapping.md). Substrate-specific attributes (GSM, media profile) have no native ERPNext equivalent; Substrate is a specialization of Material, with its exact DocType structure (child table vs. standalone) deferred to downstream design.

#### Classification
Warehouse, Stock Ledger, Item (native role): **Native.** Material, Substrate: **Customize** (Resolved — AR-006, Option C, 2026-09-26; ADR-016) — Material is a Custom PrintOS master linked to native Item; Substrate is a specialization of Material.

#### Recommendation
ERPNext owns Warehouse and stock movement mechanics entirely, and Item remains the native inventory/purchasing/UOM/accounting integration record. PrintHub implements Material as a Custom PrintOS master linked to native Item, with Substrate as a specialization of Material holding GSM/media-profile attributes (AR-006, Option C, 2026-09-26; ADR-016). Do not build a parallel stock ledger or warehouse concept.
...
| Item / Material / Substrate | ERPNext / PrintOS | Item — Native (unchanged); Material/Substrate — Customize (Resolved — AR-006, Option C; ADR-016, `Naming_Registry.md` §27 item 10 Resolved) |
...
3. **(Resolved)** Machine's ERPNext-base question is Resolved via AR-004, Option C: Machine is a wholly Custom PrintOS DocType. The Quotation/Estimation decision is Resolved via AR-005, Option B: Custom Cost Estimate and pricing logic feed ERPNext's native Quotation through a governed handoff. The Item/Material/Product Template mapping is Resolved via AR-006, Option C (ADR-016): Material is a Custom PrintOS master linked to native Item; Substrate is a specialization of Material; Product Template is its own Custom PrintOS artifact, independent of Material and Item. AR-010 remains Open and should be resolved before detailed Cost Estimate cost-breakdown design, to avoid later rework.
...
- Machine's decision is Resolved (AR-004, Option C, 2026-09-20); this document's Section 3/4/7 entries have been updated accordingly. Quotation's decision is now Resolved (AR-005, Option B, 2026-09-26); Section 7 has been updated accordingly. The Item/Material/Product Template mapping is now Resolved (AR-006, Option C, 2026-09-26; ADR-016); Section 3 (Inventory) and Section 7 have been updated accordingly.
...
- **(Resolved)** Material/Substrate/Product Template mapping settled: Material is a Custom PrintOS master linked to native Item; Substrate is a specialization of Material; Product Template is its own Custom PrintOS artifact, independent of Material and Item (AR-006, Option C, 2026-09-26; ADR-016).
...
| 0.7 | 2026-09-26 | AR-006 Disposition Synchronization | Corrected active statements following [Architecture Review Register](../decisions/Architecture_Review_Register.md) AR-006's Resolved disposition (Option C, 2026-09-26; [ADR-016-Item-Material-Product-Template-Mapping](../decisions/ADR-016-Item-Material-Product-Template-Mapping.md)): Material is a Custom PrintOS master record linked to native Item; Substrate is a specialization of Material; Product Template is its own Custom PrintOS artifact, independent of Material and Item. Updated Section 3's Inventory Reuse Opportunity, Gap Analysis, Classification, and Recommendation text; Section 7's Implementation Boundary Summary table row; Recommendation 3 (Section 8); Future Considerations; and Open Questions to record the Resolution and remove stale "provisional"/"Pending ADR" framing. Item's own native role (inventory, purchasing, unit-of-measure, accounting integration) is unchanged; Substrate's exact DocType structure (child table vs. standalone) remains deferred to a separate downstream design task and is not specified here. No other capability classification (Native/Extend/Customize/Plugin/Future) was changed. No implementation authorized. |
