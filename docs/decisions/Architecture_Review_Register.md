...
| AR-005 | Quotation Strategy | Resolved | High | No |
| AR-006 | Item vs. Material/Product Template Mapping | Resolved | High | No |
...
#### Disposition
Resolved by Project Owner selection of **Option B** (2026-09-26): Custom Cost Estimate and pricing logic feed ERPNext's native customer-facing Quotation through a governed handoff. Native Quotation remains the customer-facing document; any extension required for the handoff is deferred to downstream DocType design. Recorded by populating the existing empty Blueprint placeholder `../blueprint/14_Quotation_Engine.md`, now Draft, Version 0.1. AR-010 (BOM necessity) is not resolved by this disposition and is not a condition on it. AR-010 remains Open and should be resolved before detailed Cost Estimate cost-breakdown design, to avoid later rework; it does not resolve Quotation's own strategy. No ADR was created; this register's own Section 1 permits a Blueprint update as an alternative closure mechanism. Implementation Authorization: Not Granted.
...
#### Disposition
Resolved by Project Owner selection of **Option C** (2026-09-26), formalized in **[ADR-016-Item-Material-Product-Template-Mapping](ADR-016-Item-Material-Product-Template-Mapping.md)** (Accepted, Version 1.0): Material is a Custom PrintOS master record linked to native ERPNext Item; Substrate is a specialization of Material holding print-specific properties (GSM, media profile), with its exact structure (child table vs. standalone DocType) deferred to downstream DocType design; Product Template is its own Custom PrintOS artifact in the Estimation context, structurally independent of Material and Item. Native ERPNext Item remains the record of native inventory, purchasing, unit-of-measure, and accounting integration. This disposition also resolves [Naming Registry](../standards/Naming_Registry.md) Section 27 item 10 and the Substrate-specialization Open Question in [../blueprint/08_Master_Data_Model.md](../blueprint/08_Master_Data_Model.md), and is recorded by populating the existing empty Blueprint placeholder `../blueprint/17_Inventory_Model.md`, now Draft, Version 0.1. Unlike AR-004 and AR-005, this disposition's resolving authority is a formal numbered ADR (ADR-016), not this register's own Section 1 Blueprint-alternative-closure mechanism, because Naming Registry Section 27 item 10 required a Naming Authority decision via ADR before resolution — no Level 2 item in that Matrix has ever closed without one. AR-010 (BOM necessity) is not resolved by this disposition and is not a condition on it; it remains Open and unchanged. Implementation Authorization recorded as Not Granted; no runtime validation, Publication, or milestone completion is claimed. AR-001 through AR-005 remain Resolved as previously recorded; AR-007 through AR-011 unchanged.
...
| AR-005 Quotation Strategy | Yes (`14_Quotation_Engine.md`) | Yes (Quotation) | Yes (Workflow Designer target) | Partial |
| AR-006 Item vs. Material/Product Template | Yes (`08_Master_Data_Model.md` — resolved Open Question; `17_Inventory_Model.md` — new ownership-boundary record) | Yes (Item, Material, Substrate) | No | Partial |
...
| 0.8 | 2026-09-26 | AR-006 Resolved — Project Owner Disposition | AR-006 formally resolved by Project Owner selection of Option C on 2026-09-26, formalized in [ADR-016-Item-Material-Product-Template-Mapping](ADR-016-Item-Material-Product-Template-Mapping.md) (Accepted, Version 1.0). Section 2 Review Summary row updated: Status Open → Resolved, Blocks Development Partial → No, Priority unchanged at High. Added a Disposition block recording that Material is a Custom PrintOS master record linked to native ERPNext Item, Substrate is a specialization of Material with its exact DocType structure deferred to downstream design, and Product Template is its own Custom PrintOS artifact independent of Material and Item, recorded by populating the existing empty Blueprint placeholder `../blueprint/17_Inventory_Model.md` to Draft, Version 0.1 — no new file was created. This disposition also resolves [Naming Registry](../standards/Naming_Registry.md) Section 27 item 10 and the Substrate-specialization Open Question in `../blueprint/08_Master_Data_Model.md`, corrected there as a bounded, reference-only update (Version 1.0 → 1.1) that does not reopen that document's Frozen classification and required no fresh Architecture or Business Review; a Consistency Review and Documentation Governance check were performed and confirmed no other current-state citation is contradicted. Updated the Section 4 Cross-Review Matrix AR-006 row's Impacts Blueprint column to cite both `08_Master_Data_Model.md` (resolved Open Question) and `17_Inventory_Model.md` (new ownership-boundary record); Blocks Gap Analysis is retained at Partial, consistent with the AR-001, AR-002, AR-004, and AR-005 rows, which likewise retained their original Blocks Gap Analysis value after resolution. AR-010 (BOM necessity) is not resolved by this disposition and is not a condition on it; it remains Open and unchanged. Unlike AR-004 and AR-005, this disposition's resolving authority is a numbered ADR (ADR-016), not this register's own Section 1 Blueprint-alternative-closure mechanism, since Naming Registry Section 27's own governing rule requires a Naming Authority decision via ADR for every item in that Matrix, and no Level 2 item has ever closed there without one. The AR-006 record's own Current Situation, Why Review Is Required, Impact, Possible Options, Required Decision, Blocking Status, and Dependencies sections are preserved unchanged as historical record, per the precedent established at Version 0.4 (AR-002), Version 0.6 (AR-004), and Version 0.7 (AR-005). Implementation Authorization recorded as Not Granted; no runtime validation, Publication, or milestone completion is claimed. AR-001 through AR-005 remain Resolved as previously recorded; AR-007 through AR-011 unchanged. No other AR resolved, reprioritized, or created. |
| 0.7 | 2026-09-26 | AR-005 Resolved — Project Owner Disposition | AR-005 formally resolved by Project Owner selection of Option B on 2026-09-26. Section 2 Review Summary row updated: Status Open → Resolved, Blocks Development Partial → No, Priority unchanged at High. Added a Disposition block recording that Custom Cost Estimate and pricing logic feed ERPNext's native customer-facing Quotation through a governed handoff, recorded by populating the existing empty Blueprint placeholder `../blueprint/14_Quotation_Engine.md` to Draft, Version 0.1 — no new file was created. Handoff mechanics, fields, Custom Fields, child-table structure, triggers, validations, and Quotation Line's target treatment remain deferred to a separate downstream DocType-design task. Updated the Section 4 Cross-Review Matrix AR-005 row's Impacts Blueprint column from No to Yes (`14_Quotation_Engine.md`); Blocks Gap Analysis is retained at Partial, consistent with the AR-001, AR-002, and AR-004 rows, which likewise retained their original Blocks Gap Analysis value after resolution. AR-010 (BOM necessity) is not resolved by this disposition and is not a condition on it: AR-010 remains Open and should be resolved before detailed Cost Estimate cost-breakdown design, to avoid later rework; it is not treated as mandatory, resolved, a new module-level blocker, or a condition on this decision. The AR-005 record's own Current Situation, Why Review Is Required, Impact, Possible Options, Required Decision, Blocking Status, and Dependencies sections are preserved unchanged as historical record, per the precedent established at Version 0.4 (AR-002) and Version 0.6 (AR-004). No ADR was created; this register's own Section 1 permits a Blueprint update as an alternative closure mechanism. Implementation Authorization recorded as Not Granted. AR-001 through AR-004 remain Resolved as previously recorded; AR-006 through AR-011 unchanged. No other AR resolved, reprioritized, or created. |
