...
Lifecycle position of Version 1.4:
**Draft, Version 1.4 (2026-09-12).** Version 1.4 records the Project Owner's adopted **Model 4** Artwork-applicability design direction (Section 2.1) — **new normative design content that has received no Architecture Review and no Business Review**. Under `../Documentation_Workflow.md` Section 5, which defines **Approval** as "All required reviews passed; awaiting final Owner sign-off," and Section 7, which prohibits reaching Approval "until every applicable review lens has been explicitly passed," this document **returns to Draft at Version 1.4**; Sections 6 and 11 keep Project Owner approval distinct from those lenses, so Project Owner authorization alone does not satisfy them. Workflow Section 8 independently confirms **MINOR** as the correct increment for an additive, non-contradictory expansion. This treatment follows the governed precedent already applied to this document at Version 1.1 (2026-08-13) and to `../implementation/JobCard_TierA_System_Design.md` at Version 1.5 (2026-08-13, by explicit Project Owner ruling). **Targeted Architecture Review and Business Review of Section 2.1 are Pending.** No rule recorded in the Approval, Version 1.3 baseline is changed, reversed, or weakened by Version 1.4.

Current Approved Baseline:
**Approval, Version 1.3 (2026-08-22).** Version 1.3 is a **bounded naming-status and candidate synchronization** of Version 1.2 — it records that the five Artwork technical DocType names are now Naming Registry **Proposed** (per Project Owner decision dated 2026-08-22; see `../standards/Naming_Registry.md` Section 13a), and that `PrintHub Customer Approval Evidence` is now the first documented provisional technical-name candidate for the existing, already-governed Customer Approval Evidence entity. **No lifecycle, authority, cardinality, approval, evidence, uniqueness, revocation, Set, Job Card, or gate rule changed; no normative Artwork rule, business decision, or design content changed.** None of the five names is Approved; Naming Registry Under Review has not begun; source-based collision verification remains unauthorized and outstanding. Version 1.2's Project Owner Document Lifecycle Approval (granted 2026-08-19 at Version 1.1, carried forward through Version 1.2's bounded reference-only correction on 2026-08-22) carries forward unchanged and is preserved at Version 1.3.

**Prior historical baseline:** Approval, Version 1.2 (2026-08-22), which itself carried forward Approval, Version 1.1 (2026-08-19) and Approval, Version 1.0 (2026-07-31) unchanged; every rule recorded in the 1.0/1.1/1.2 baseline is preserved unchanged in 1.3, which updates naming-status citations only.

**Bounded meaning of this approval.** This approval accepts the current architecture and business-design baseline only. It does **not** grant or imply: Publication; direct coding authority; source-inspection authorization; implementation authorization; environment creation; runtime validation; production authorization or readiness; closure of any of the 62 Artwork runtime gates; closure of the Artwork production gate; global lock-order resolution; or Full Architecture Freeze. Separately and for clarity — **`AR-003` (Architecture Review Register) concerns only its own recorded eight terms and is unaffected by, and irrelevant to, this approval**; **the five Artwork technical DocType names (Section 3) are Naming Registry Proposed only, not Approved, under Naming Registry §§5, 26, 38 and 39** (not governed by AR-003; Under Review has not begun; source-based collision verification remains unauthorized and outstanding); and **selection of the `printos_core` module path remains open, with no current governance mechanism or candidate**. None of these three items is resolved, approved, or established by this approval. `../database/Artwork_Authority_DocType_Specification.md` remains **Draft**, **not Published**, and **not safe for coding**.
...
- **Project Owner Document Lifecycle Approval (2026-08-19).** The Project Owner explicitly granted lifecycle Approval on **2026-08-19**, superseding the "remains Draft, Version 1.1" status recorded in the bullets above (which correctly describe this document's state as of the dated actions they each describe). This document became **Approval, Version 1.1** at that time, and — following a bounded reference-only correction on **2026-08-22** and this same day's bounded naming-status synchronization (see Revision History) that changed no normative content — is now **Approval, Version 1.3** (see the Document Control header above); **Approval, Version 1.2**, **Approval, Version 1.1**, and, before them, **Approval, Version 1.0** are the prior historical baselines. This approval accepts the current architecture and business-design baseline only and grants none of the authorities enumerated in the header's "Bounded meaning of this approval" note; the Artwork production gate, all 62 runtime gates, AR-003, and every other item listed there remain exactly as recorded elsewhere in this section.
...
## 2.1 Artwork Applicability Design Direction — Model 4 (Project Owner decision 2026-09-12)
...
This section records a **high-level design direction only**. It is an **approved design input** awaiting its review lenses — it is not Architecture Review acceptance, not Business Review acceptance, not document lifecycle Approval, not Publication, and not implementation authorization.

**Governed principles of the adopted direction:**

1. Artwork Authority remains **shared governed functionality within `printos_core`** — one governed custom app, per Naming Registry Section 13h. It is not a separately forked codebase and not an inconsistent per-Tenant code installation.
2. **Tenant enablement** controls whether the Artwork Authority capability is available for that Tenant. This is capability configuration, not a code-level variation.
3. When the capability is enabled, a **future governed and auditable per-order applicability decision** determines which eligible Sales Orders or production work require Artwork Authority.
4. Work classified as **Artwork-controlled remains subject to every existing rule** in this document without exception — Revision lifecycle, customer approval, Customer Approval Evidence, Production Artwork Set, Set Item membership, withdrawal, supersession, revocation and atomic propagation, and Job Card binding.
5. **"Not applicable" is not an informal waiver, manual override, or permission bypass.** It can only ever be the outcome of the governed applicability decision described above.
6. **Applicability history must not be silently rewritten** after it has governed production authority.
7. A **controlled non-Artwork release path** is required for work validly classified outside Artwork Authority.
8. **That non-Artwork release path and its executable prerequisites remain undefined and unauthorized.** Nothing in this section creates one.
9. **No existing record is automatically migrated, rebound, or reclassified** by this decision.
10. **Model 4 is not safe for coding** until its remaining rules are governed, reviewed, and independently verified.

**Relationship to the currently governed rules.** Section 9.1's completeness predicate — including its requirement of at least one required Artwork — and Section 5.1's mandatory customer approval before internal production approval remain **exactly as approved in the Approval, Version 1.3 baseline**. This section does not relax, suspend, or create an exception to either. Any future change to the governing Artwork prerequisite must be made through a separate Architecture Review, Business Review, and Project Owner approval, as required by `../implementation/JobCard_TierA_System_Design.md` Section 10.1. Because such a change would reverse an existing business rule, `../Documentation_Workflow.md` Section 8 would additionally require a **MAJOR** version increment and the full Review and Approval cycle at that time.

**Unresolved rule-definition requirements.** The following remain **undefined and unresolved**, and no answer to any of them is supplied, implied, or reserved by this decision: the authoritative Tenant enablement record; the authoritative per-order applicability record; the exact governing object level (Sales Order, Sales Order Item, Job Card, or another governed level); defaults for new and existing Tenants; authorized actors and the permission model; decision timing and the lock point after which applicability may not change; the controlled correction procedure; required actor, timestamp, reason, and audit history; treatment of submitted Sales Orders; treatment of Registered, Released, In Progress and Completed Job Cards; treatment of existing Production Artwork Set bindings; the exact non-Artwork release prerequisites; user-interface explanation and warnings; reporting and exception monitoring; import and bulk-update controls; migration and rollback behavior; and transaction, locking, concurrency, and validation behavior.

---

# 3. Naming Boundary

The following five technical names are **Naming Registry Proposed** (per Project Owner decision dated 2026-08-22; `../standards/Naming_Registry.md` Section 13a) — **none is Approved; Under Review has not begun; source-based collision verification remains outstanding and unauthorized**:
...
| Customer Approval Evidence | `PrintHub Customer Approval Evidence` — first documented provisional technical identity for this existing, already-governed entity (Section 6.2a); the entity and its business rules are unchanged |
| Production Artwork Set | `PrintHub Production Artwork Set` |
| — (set membership row) | `PrintHub Production Artwork Set Item` |

No claim is made that these names are Approved by the Naming Registry. This document does not modify the Naming Registry beyond citing its Section 13a Proposed entries, and `AR-003` is neither resolved nor modified.
...
| Field type | Link → `PrintHub Production Artwork Set` (Naming Registry status: Proposed, not Approved) |
...
- [../implementation/JobCard_TierA_System_Design.md](../implementation/JobCard_TierA_System_Design.md) — the consuming Production design; currently **Approval, Version 1.7** (Project Owner Document Lifecycle Approval granted 2026-08-19 at Version 1.5, an independent decision from this document's own approval; carried forward through its own bounded reference-only correction and naming-status synchronization on 2026-08-22), prior historical baselines **Approval, Version 1.6**, **Approval, Version 1.5**, and **Approval, Version 1.4**. Its Version 1.5 content records the `production_artwork_set` reference contract reconciled from this document's Approval 1.0 baseline (Sections 13, 16.1, 16.2); this document's own Version 1.1 content (Section 7.2) is not relied upon by that contract.
...
|---|---|---|---|
| 1.4 | 2026-09-12 | Project Owner Decision — Model 4 Artwork Applicability Design Direction (A4, B2, C2) | **Lifecycle treatment.** Adds new Section 2.1 recording the Project Owner's adopted Model 4 design direction — Tenant enablement with governed per-order applicability — together with the acceptance of the independently verified Architecture Review and Business Review advisory dispositions **subject to** their verified corrections and remaining rule definition. Because Section 2.1 is **new normative design content that has received no Architecture Review and no Business Review**, this document **returns to Draft at Version 1.4** under `../Documentation_Workflow.md` Section 5 (Approval means "All required reviews passed; awaiting final Owner sign-off") and Section 7 (Approval unreachable until every applicable lens has explicitly passed), with Sections 6 and 11 keeping Project Owner approval distinct from those lenses. Workflow Section 8 confirms **MINOR** as the correct increment for an additive, non-contradictory expansion. This mirrors the governed precedent applied to this document at Version 1.1 (2026-08-13) and to `../implementation/JobCard_TierA_System_Design.md` at Version 1.5 (2026-08-13, by explicit Project Owner ruling). **The Approval, Version 1.3 baseline (2026-08-22) remains the last fully reviewed and Owner-approved version**, and every rule it contained is preserved unchanged; no earlier approved version contained Model 4. **Targeted Architecture Review and Business Review of Section 2.1 are Pending.** **Content of this revision.** Section 2.1 records ten governed principles: Artwork Authority remains shared governed functionality in `printos_core`; Tenant enablement controls capability availability; a future governed, auditable per-order applicability decision determines which eligible work requires Artwork Authority; Artwork-controlled work remains subject to every existing approval, Revision, Evidence, Set, Set Item, withdrawal, supersession and Job Card binding rule; "not applicable" is not an informal waiver, manual override or permission bypass; applicability history must not be silently rewritten after governing production authority; a controlled non-Artwork release path is required for validly non-applicable work; that path and its executable prerequisites **remain undefined and unauthorized**; no existing record is automatically migrated, rebound or reclassified; and Model 4 is **not safe for coding** until its remaining rules are governed, reviewed and independently verified. Section 2.1 further records seventeen **unresolved rule-definition requirements** and supplies no answer to any of them. **Section 9.1's completeness predicate and Section 5.1's mandatory customer approval are unchanged and unrelaxed**; no exception to either is created. **Review and finding treatment.** Conditional acceptance of an advisory disposition **does not complete a governed review lens**: Architecture Review and Business Review of the six technical names both remain **Pending**, and all twelve independently verified findings (`ART-NAR-F1`–`F5`, `ART-NBR-F1`–`F5`, `ART-TLV-F1`–`F2`) remain **pending — none is Closed**, since none has completed correction plus independent verification. `ART-NBR-F5` is addressed at direction level only; its operative rules remain undefined. These are local review labels, **not** Architecture Review Register identifiers; **no AR identifier was created and `AR-003` was not modified or resolved**. **Scope.** Documentation-only. All six technical names remain **Under Review**; none is Approved. `ART-RVR-C08` remains incomplete; `ART-RVR-B02` remains stable and design/governance-resolved. No field, setting, DocType, API, role, permission, default, migration, workflow transition or runtime rule was created. No null `production_artwork_set` release behavior is authorized. All 62 runtime gates and the Artwork production gate remain **Open**; no Full Architecture Freeze is claimed; Publication, implementation, deployment and runtime-validation authority are **not** granted. `../database/Artwork_Authority_DocType_Specification.md` remains **Draft**, **not Published**, and **not safe for coding**. |
| 1.3 | 2026-08-22 | Naming-Status and Candidate Synchronization | **Bounded naming-status and candidate synchronization only; Approval is retained.** Per explicit Project Owner decision dated 2026-08-22, the five Artwork technical DocType names (`PrintHub Artwork`, `PrintHub Artwork Revision`, `PrintHub Production Artwork Set`, `PrintHub Production Artwork Set Item`) entered Naming Registry **Proposed** status (`../standards/Naming_Registry.md` Section 13a), and **`PrintHub Customer Approval Evidence` was introduced as the first documented provisional technical-name candidate** for the existing, already-governed Customer Approval Evidence entity (Section 6.2a) — the entity and its business rules are unchanged; only a provisional technical identity is added. Updated Section 3 (Naming Boundary), the header "Bounded meaning of this approval" clause, Section 2, Section 6.2a, Section 21 preamble, and the Section 14 field-type reference to state that these names are Proposed, not Approved, and that Naming Registry Under Review has not begun and source-based collision verification remains unauthorized and outstanding. Synchronized the Job Card Tier A System Design citation to **Approval, Version 1.7**. **No lifecycle, authority, cardinality, approval, evidence, uniqueness, revocation, Set, Job Card, or gate rule changed; no normative Artwork rule, business decision, or design content changed.** `AR-003` was not resolved or modified and remains unaffected. No `printos_core` module path was selected or established. This correction grants no source-inspection, implementation, Publication, or runtime-validation authority; all 62 Artwork runtime gates and the Artwork production gate remain **Open**. No AR identifier was invented. |
| 1.2 | 2026-08-22 | AR-003 Scope Misattribution Correction | Bounded reference-only correction, per `../Documentation_Workflow.md` Section 8 ("Minor Updates — Clarifications... non-contradictory expansions... incrementing the MINOR version"). A repository-wide census found that this document's header ("Bounded meaning of this approval") and two body clauses (Section 6.2a Customer Approval Evidence naming; Section 20 Exclusions) incorrectly attributed Artwork/Job Card technical-name registration and `printos_core` module-path selection to **`AR-003`**. Direct inspection of `../decisions/Architecture_Review_Register.md` confirms `AR-003`'s actual recorded scope is **only** its eight named terms ("Print Specification," "Approval Management," "Production Workflow," "Machine Management," "Finishing," "Quality Control," "AI Assistant," "Production Orchestration") — none of which is Artwork- or Job Card-related. **Corrected all three current-state occurrences** to state plainly: `AR-003` concerns only its own eight terms and remains Open globally, unaffected by and irrelevant to this document; registration of the five proposed Artwork/Job Card technical DocType names is a **separate, currently untracked** question governed by Naming Registry §§5, 26, 38 and 39 (not §33, since these are net-new terms, not renames of an existing Approved term); and selection of the `printos_core` module path is **separately unresolved, with no current governance mechanism or candidate structure documented anywhere** in the repository. **No technical name or module path is selected or approved by this correction.** `AR-003` itself was **not** modified, resolved, or reinterpreted — `docs/decisions/Architecture_Review_Register.md` was not touched by this task. **No normative Artwork business rule, lifecycle state, Set/Revision behavior, or Job Card contract was changed.** Synchronized the companion `database/Artwork_Authority_DocType_Specification.md` (→ Draft 0.9), `implementation/JobCard_TierA_System_Design.md` (→ Approval 1.6), `database/JobCard_TierA_DocType_Specification.md` (→ Draft 0.9), `reviews/Artwork_Runtime_Validation_Readiness_Register.md` (→ Draft 0.2), `database/Business_Entity_Inventory.md` (→ Draft 0.4), `blueprint/00_Master_Index.md` (→ Draft 2.6), and `Documentation_Status.md` (→ Draft 1.24) for the identical correction and consistent cross-references. **Four historical revision-history entries elsewhere containing the same misattribution (this document's own Version 1.1 row below; Job Card System Design's Version 1.5 row; Business Entity Inventory's Version 0.3 row; a Documentation Status revision row) were deliberately preserved unchanged** and are instead annotated prospectively in their respective documents' new revision-history rows, per Section 8's rule that the Revision History table is "updated at the same time as the content change, never retroactively." **This correction grants no source-inspection, implementation, Publication, or runtime-validation authority**; all 62 Artwork runtime gates and the Artwork production gate remain **Open**; no gate is closed; Full Architecture Freeze remains unachieved. No AR identifier was invented; AR-003 was not resolved or modified. |
