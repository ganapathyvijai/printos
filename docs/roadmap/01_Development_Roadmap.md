...
- **Job Card Tier B (the operationally schedulable Job Card) is the principal convergence point** of the dependency graph, where AR-005 and AR-006 consequences meet (AR-004's Machine-Scheduling consequence Resolved, 2026-09-20) (Matrix §8; Section 15).
...
| Production Planning | Production Operations | Conditional Roadmap Scope | Conditional Architecture | AR-005/006 (transitive/convergent via inputs; AR-004 exposure Resolved) | Define Conditional Workstream | Conditional | Not Authorized | Distinct module (ADR-014); consumes, does not create Job Card |
| Job Cards | Production Operations | Conditional Roadmap Scope (Tier A / Tier B gates) | Conditional Architecture | AR-006 (convergent, Tier B), AR-005 (transitive), AR-009; AR-004 convergent exposure Resolved | Define Conditional Workstream | Conceptually Ready (Tier A) / Conditional (Tier B) | Not Authorized | One aggregate, one DocType; tiers are readiness gates |
...
- Remains **Conditional** — the convergence point where **AR-006** (Material, via allocation) and **AR-005** (Quotation, transitive via Sales Order) meet; AR-004's Machine/Machine Scheduling contribution is Resolved (Option C, 2026-09-20). This is **convergent blocking**, not direct blocking of the Job Card entity.
...
**Traceability:** ADR-014; `blueprint/09_PrintOS_Modules.md`; `Canonical_Domain_Model.md`; `ERPNext_DocType_Mapping.md` (single Job Card row); AR-004 (Resolved)/005/006/009; Matrix §6–9.
...
- **AR-005 through AR-011** — all Open (Section 10; register verbatim). **AR-001** — Resolved (Option A; ERPNext v16/Frappe v16 governed target, reaffirming Accepted ADR-001); capability-level technical revalidation remains outstanding and is outside this roadmap's scope. **AR-002** — Resolved (Option A; one isolated Frappe site and operational database per Tenant, Tenant distinct from Company, per Accepted ADR-015, Version 1.0); implementation design, Publication of `25_MultiTenant_Architecture.md`, and separate scoped authorization remain outstanding and are outside this roadmap's scope. **AR-003** — Resolved (Option A, 2026-09-19; seven terms mapped/confirmed/rejected against existing Approved concepts, AI Assistant registered as Proposed only); AI Assistant's architecture, Bounded Context, provider, and model remain outstanding, pending separate future governance, and are outside this roadmap's scope. **AR-004** — Resolved (Option C, 2026-09-20; Machine and Machine Profile are wholly Custom PrintOS DocTypes, per `16_Print_Machine_Model.md`); Blueprint review completion and a separate scoped implementation-authorization decision remain outstanding and are outside this roadmap's scope.
